...
Render factory trigger update of the DOM by the `observe` method of the descriptor. It means that update is scheduled with internal queue and executed in the next animation frame. The passed `fn` is always called for the first time and when related properties change.
